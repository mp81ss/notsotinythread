...
/// Semaphore class.
/// This is a classic synchronization primitive.
class semaphore
{
public:
  ...
  /// Tell if last operation on semaphore went ok.
  ...
  inline void wait() {
#ifdef _TTHREAD_WIN32_
      valid_ =  WaitForSingleObject(semaphore_, INFINITE) == WAIT_OBJECT_0;
#else
      valid_ = sem_wait(&semaphore_) == 0;
#endif
  }
  ...
  inline void post() {
#ifdef _TTHREAD_WIN32_
    valid_ =  ReleaseSemaphore(semaphore_, 1, NULL) != 0;
#else
    valid_ = sem_post(&semaphore_) == 0;
#endif
  }
...
public:
  ...
  /// If calling thread is the last thread that was waiting on the barrier,
  /// it unblocks all other threads that were waiting on the barrier,
  /// otherwise it blocks on the barrier.
  /// \return ZERO if calling thread blocks, NON-ZERO if it was the \b last
  /// thread and unblocked all other threads.
  int wait();

  /// Tell if last operation on barrier went ok.
  /// \return true if all ok, false otherwise.
  inline bool is_valid() const {
    return valid_;
  }
